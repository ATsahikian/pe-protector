  ...
  for (unsigned int i = 0; i < commands.size(); ++i) {
    if (commands[i].mNameLabel == label ||
        (commands[i].mType == NCommand::DATA &&
         commands[i].mData.mName == label))  // todo check case sensitive
    {
      return i;
    }
  }
  throw std::runtime_error(std::string{"label \""} + label +
                           "\" doesn't exist");
}
...
SOperand createOperandMemory(const vector<SCommand>& commands,
  ...
  if (isMatch(tokens,
              {NCategory::CONSTANT, NCategory::ASTERIX, NCategory::REGISTER,
               NCategory::PLUS, NCategory::REGISTER})) {
    ...
    position += 5;
  } else if (isMatch(tokens, {NCategory::REGISTER, NCategory::PLUS,
                              NCategory::REGISTER})) {
    result.mMemory.mRegisters.push_back(NRegister::EType(tokens[0].mIndex));
    result.mMemory.mRegisters.push_back(NRegister::EType(tokens[2].mIndex));
    position += 3;
  } else if (isMatch(tokens, {NCategory::CONSTANT, NCategory::ASTERIX,
                              NCategory::REGISTER})) {
    // result.mMemory.mScale = getScale(tokens[0].mConstant);
    result.mMemory.mScale = tokens[0].mConstant;
    result.mMemory.mRegisters.push_back(NRegister::EType(tokens[2].mIndex));
  ...
  if (position < tokens.size()) {
    result.mMemory.mConstant =
        createConstant(commands, getSubTokens(tokens, position));
  }
  return result;
}
...
SOperand createOperand(const vector<SCommand>& commands,
  ...
  } else if (tokens.size() > 4 && tokens[0].mType == NCategory::DATA_TYPE &&
  ...
  else if (tokens.size() > 6 && tokens[0].mType == NCategory::DATA_TYPE &&
           tokens[1].mType == NCategory::PTR &&
...
void fillInstruction(SCommand& command,
  ...
  if (!tokens.empty()) {
    ...
    do {
      ...
      command.mInstruction.mOperands.push_back(createOperand(
          commands,
          getSubTokens(tokens, beginPosition /*beginPosition*/,
                       (commaPosition == -1)
                           ? commaPosition
                           : commaPosition - beginPosition /*length*/)));
...
vector<SCommand> compile(basic_istream<char, char_traits<char> >& input) {
  ...
  for (unsigned int indexLine = 0; indexLine < tokens.size(); ++indexLine) {
    ...
    } else if (isMatch(tokens[indexLine],
                       {NCategory::DIRECTIVE, NCategory::NAME})) {
      result.push_back(
          createDirective(currentLabel, indexLine, tokens[indexLine][1].mData));
      currentLabel.clear();
    }
    // else if (isMatch(tokens[indexLine], {NCategory::IMPORT, NCategory::NAME,
    // NCategory::DOT, NCategory::NAME}))
    else if (isMatch(tokens[indexLine], {NCategory::IMPORT, NCategory::NAME})) {
      // result.push_back(createImport("", indexLine,
      // tokens[indexLine][1].mData, tokens[indexLine][3].mData));
      result.push_back(createImport("", indexLine, tokens[indexLine][1].mData));
    } else if (isMatch(tokens[indexLine],
                       {NCategory::EXTERN, NCategory::DATA_TYPE,
                        NCategory::NAME})) {
      result.push_back(createExtern(
          "", indexLine, NDataType::EType(tokens[indexLine][1].mIndex),
          tokens[indexLine][2].mData));
    } else if (isMatch(
                   tokens[indexLine],
                   {NCategory::SECTION, NCategory::STRING, NCategory::NAME})) {
      result.push_back(createSection("", indexLine, tokens[indexLine][1].mData,
                                     tokens[indexLine][2].mData));
    } else if (isMatch(tokens[indexLine],
                       {NCategory::NAME, NCategory::COLON, NCategory::PREFIX,
                        NCategory::INSTRUCTION})) {
      if (!currentLabel.empty()) {
        // warning!
        currentLabel.clear();
      }
      result.push_back(
          createInstruction(tokens[indexLine][0].mData, indexLine,
                            NPrefix::EType(tokens[indexLine][2].mIndex),
                            NInstruction::EType(tokens[indexLine][3].mIndex)));
      ...
      result.push_back(
          createInstruction(tokens[indexLine][0].mData, indexLine, NPrefix::NON,
                            NInstruction::EType(tokens[indexLine][2].mIndex)));
    } else if (isMatch(tokens[indexLine],
                       {NCategory::PREFIX, NCategory::INSTRUCTION})) {
      result.push_back(createInstruction(
          currentLabel, indexLine, NPrefix::EType(tokens[indexLine][0].mIndex),
          NInstruction::EType(tokens[indexLine][1].mIndex)));
      currentLabel.clear();
    } else if (isMatch(tokens[indexLine], {NCategory::INSTRUCTION})) {
      result.push_back(
          createInstruction(currentLabel, indexLine, NPrefix::NON,
                            NInstruction::EType(tokens[indexLine][0].mIndex)));
      currentLabel.clear();
    } else if (isMatch(tokens[indexLine],
                       {NCategory::NAME, NCategory::DATA_TYPE})) {
      result.push_back(
          createData(currentLabel, indexLine, tokens[indexLine][0].mData,
                     NDataType::EType(tokens[indexLine][1].mIndex)));
      currentLabel.clear();
    } else if (isMatch(tokens[indexLine],
                       {NCategory::NAME, NCategory::COLON})) {
      // save label !
  ...
  for (unsigned int indexLine = 0, indexCommand = 0; indexLine < tokens.size();
    ...
    if (!lineTokens.empty()) {
      if (isMatch(lineTokens, {NCategory::PREFIX, NCategory::INSTRUCTION})) {
        fillInstruction(result[indexCommand], result,
                        getSubTokens(lineTokens, 2));
      } else if (isMatch(lineTokens, {NCategory::INSTRUCTION})) {
        fillInstruction(result[indexCommand], result,
                        getSubTokens(lineTokens, 1));
